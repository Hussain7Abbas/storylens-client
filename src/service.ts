...
const requestSchema = z.object({ prompt: z.string().min(1), model: z.string().min(1).max(200), effort: z.string().min(1).max(20), responseLanguage: z.enum(["en", "ar"]) }).strict();
...
export class PromptService {
  ...
  async refresh(): Promise<Capabilities> {
    ...
    this.capabilities = { protocolVersion: 2, models: catalog.models, providers: catalog.providers, limits: LIMITS };
  ...
  async executePrompt(input: unknown, signal?: AbortSignal): Promise<ExecuteOutput> {
    ...
    const { prompt, model: requestedModel, effort, responseLanguage } = parsed.data;
    ...
    try {
      const output = await this.execute(this.settings.get(), model, effort, prompt, responseLanguage, controller.signal);
